...
int iter2(int tab[], int j, int n, int highnum, size_t size) {
	if (j > size) {
		return highnum;
	} else {
		int diff = tab[j] - tab[n]; 
		highnum = (diff > highnum) ? diff : highnum;
		printf("High num in iter2: %d \n", highnum);  
	}
	iter2(tab, j + 1, n, highnum, size - 1); 
}
...
int day(int tab[], size_t size) {
	...
	while (n < size) {
		int j = n + 1;
		/* printf("About to enter iter fn\n"); */
		int highnum = iter2(tab, j, n, highnum, size);

		profit = (highnum > profit) ? highnum : profit;
		printf("Profit in main: %d \n", profit);
		n++; 
	}
	return profit; 
}
